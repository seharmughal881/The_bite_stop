...
export default function NewProductsSection() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        // Fetch all bakery products from API
        const response = await productsService.getAllBakery();

        // Check if response has products array or if it's the direct array
        const bakeryData = response.products || response;

        if (!Array.isArray(bakeryData)) {
        ...
        const flavors = [
          ...new Set(
            bakeryData.map((product) => product.specs?.flavor).filter(Boolean)
          ),
        ];
        setUniqueFlavors(flavors);

        // Enhance products with images array and calculate discount percentages
        const enhancedProducts = bakeryData.map((product) => ({
          ...product,
        ...
        setError("Failed to load bakery products. Please try again later.");
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  // Filter products by flavor, discount, AND category
  const filteredProducts = products.filter((product) => {
    ...
    return flavorMatch && discountMatch && categoryMatch;
  });
    ...
    <div className="mx-auto py-8 px-6 md:px-10 relative overflow-hidden">
      ...
      >
        <div
          ...
          style={{
            backgroundImage:
              "url('https://images.unsplash.com/photo-157898555062-69928b1d9587?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1980&q=80')",
          }}
        ></div>
      ...
      >
        <div className="relative group mb-6 md:mb-0 text-center md:text-left">
          ...
          >
            Handcrafted with premium ingredients and love
          </motion.p>
        </div>
                ...
                >
                  {categoryFilterOptions.map((option) => (
                    <button
                      ...
                      className={`block w-full text-left px-4 py-2 text-sm transition-colors duration-200 ${categoryFilter === option.value
                ...
                >
                  ...
                  <div className="relative overflow-hidden rounded-t-2xl">
                      ...
                      <motion.img
                        src={
                          product.images && product.images.length > 0
                            ? product.images[0]
                            : product.imageUrl || "/placeholder-cake.jpg"
                        }
                        alt={product.title}
                    ...
                    {product.discountPercentage > 0 && (
                      <motion.div
                        className="absolute bottom-4 right-4 bg-gradient-to-r from-[#db1356] to-[#d4587a] text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md z-10"
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        transition={{ delay: index * 0.1, type: "spring" }}
                        whileHover={{ scale: 1.1, y: -2 }}
...
export function AllBakeryProducts() {
  ...
  return (
    <div className="mx-auto px-8 md:mt-15 mt-2 py-12 relative">
      {/* Chocolate-themed background with actual image */}
      <div className="absolute inset-0 -z-10 overflow-hidden">
        <div
          className="absolute inset-0 bg-cover bg-center opacity-20"
          style={{
            backgroundImage:
              "url('https://images.unsplash.cm/photo-1578985545062-69928b1d9587?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1980&q=80')",
          }}
        ></div>
