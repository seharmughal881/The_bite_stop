  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const response = await productsService.getAllCakes();

        // Check if response has products array or if it's the direct array
        const cakesData = response.products || response;
        ...
        const flavors = [
          ...new Set(
            cakesData.map((product) => product.specs?.flavor).filter(Boolean)
          ),
        ];
        setUniqueFlavors(flavors);

        // Enhance products with images array and calculate discount percentages
        const enhancedProducts = cakesData.map((product) => ({
          ...product,
    ...
    <div className="mx-auto py-8 px-6 md:px-10 relative overflow-hidden">
      ...
      >
        <div
          ...
          style={{
            backgroundImage:
              "url('https://imges.unsplash.com/photo-1578985545062-69928b1d9587?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1980&q=80')",
          }}
        ></div>
...
export function AllCakesProducts() {
    ...
    <div className="mx-auto px-8 md:mt-15 mt-2 py-12 relative">
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        <div
          ...
          style={{
            backgroundImage:
              "url('https://images.unsplash.com/phot-1578985545062-69928b1d9587?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1980&q=80')",
          }}
        ></div>
